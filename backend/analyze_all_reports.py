...
# Excel file path
EXCEL_FILE = "/Users/hashim.umarji/Downloads/Master Scout Reports (1).xlsx"
...
def load_player_mappings():
    ...
    print(f"✓ Loaded {len(mappings)} manual player name mappings\n")
    return mappings
...
def find_player_with_mapping(cursor, player_name, all_players_cache, player_mappings):
    ...
    if player_name in player_mappings:
        ...
        print(f"    Using manual mapping: '{player_name}' → '{mapped_name}'")
...
def analyze_reports(conn, reports_df, player_mappings):
    ...
    if not all(k in col_map for k in ['player', 'fixture']):
        print("✗ ERROR: Could not detect required columns (player, fixture)")
        return results
    ...
    print(f"  ✅ Can be imported: {can_count} ({can_count/total*100:.1f}%)")
    print(f"  ❌ Cannot be imported: {cannot_count} ({cannot_count/total*100:.1f}%)")
    ...
    print(f"  ✓ Saved final_analysis_summary.txt")
    ...
    if results['can_import']:
        ...
        print(f"  ✓ Saved can_import.txt ({len(results['can_import'])} reports)")
        ...
        with open('cannot_import.txt', 'w') as f:
            ...
            for report in results['cannot_import']:
                ...
                for error in report['errors']:
                    f.write(f"    - {error}\n")
                f.write("\n")
        print(f"  ✓ Saved cannot_import.txt ({len(results['cannot_import'])} reports)")
    ...
    if results['missing_players']:
        with open('still_missing_players.txt', 'w') as f:
            ...
            for player in sorted(results['missing_players']):
                f.write(f"{player}\n")
        print(f"  ✓ Saved still_missing_players.txt ({len(results['missing_players'])} players)")
    ...
    if results['missing_fixtures']:
        with open('still_missing_fixtures.txt', 'w') as f:
            ...
            for fixture in sorted(results['missing_fixtures']):
                f.write(f"{fixture}\n")
        print(f"  ✓ Saved still_missing_fixtures.txt ({len(results['missing_fixtures'])} fixtures)")
    ...
    if results['fuzzy_matches']:
        with open('fuzzy_matches_used.txt', 'w') as f:
            ...
            for match in results['fuzzy_matches']:
                ...
                f.write("-" * 80 + "\n\n")
        print(f"  ✓ Saved fuzzy_matches_used.txt ({len(results['fuzzy_matches'])} fuzzy matches)")
