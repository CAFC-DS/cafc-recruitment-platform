...
Idea: run the backend twice — once on the LEGACY seam config, once FLIPPED to
the cloned CAFC_DB.APP (or CAFC_DB.APP_DRYRUN in the dry run) — capturing the same
endpoints each time. diff.py then proves the cloned schema returns the same data
as legacy RECRUITMENT_TEST.PUBLIC, for every role. See README.md for full env.

Workflow
--------
1. Start backend on the legacy config, then capture:
       python tools/cutover_compare/capture.py --label base \
              --creds-file tools/cutover_compare/creds.json

2. Restart backend flipped (dry run):
       SNOWFLAKE_PROD_DATABASE=CAFC_DB SNOWFLAKE_PROD_SCHEMA=APP_DRYRUN \
       CANONICAL_DB=CAFC_DB PLATFORM_DB_SCHEMA=APP_DRYRUN CORE_DB_SCHEMA=APP_DRYRUN python main.py
   Capture again:
