...
- `core_table()` is only equivalent to `write_table()` while `WRITE_DB` is unset (true in production today). If `WRITE_DB` were ever set again for the documented soak state (writes held on the legacy database while reads flip to canonical), the two helpers would diverge — and for the 13 tables in this batch, that lever no longer applies to their writes, since this migration moved their write call sites off `write_table()` onto `core_table()`. Rollback for this batch is `git revert` + redeploy only, not an env-var flip.
...
Expected: 34 failed, 61 passed (58 pre-existing passes + the 3 `test_core_table_helper.py` tests from Task 1, updated post-review to pin/diverge the helpers via monkeypatch instead of comparing two unpinned defaults). If the failure count is higher than 34, one of Tasks 4-8's renames broke something the pre-existing suite does happen to cover correctly — investigate before proceeding. If it's exactly 34 failed, the new failures are the same pre-existing ones (unrelated to this work, per Global Constraints) and this batch introduced no new breakage by this measure.
