...
• "Show me match coverage statistics"`,
  ...
  const renderResults = (message: Message) => {
    ...
    return (
      <div className="mt-3">
            ...
            <thead className="table-dark">
              <tr>
                {Object.keys(results[0] || {}).map((col) => (
                  <th key={col} style={{ fontSize: "0.8rem" }}>
                    {col}
                  </th>
                ))}
              </tr>
        ...
        {message.rowCount! > 10 && (
          <small className="text-muted">
            Showing first 10 of {message.rowCount} results
          </small>
        )}
      </div>
    );
  ...
  return (
    <Card
      className="shadow-sm"
      style={{ borderRadius: "12px", border: "2px solid #6f42c1" }}
    >
      <Card.Header
        style={{
      ...
      <Collapse in={isExpanded}>
        ...
        >
          ...
          {healthStatus && (
            <Alert
              variant={getHealthStatusColor(healthStatus)}
              className="mb-3 py-2"
            >
          ...
          >
            ...
            >
              ⚠️ Experimental AI feature - verify important insights with actual
              data
            </Form.Text>
          </Form>
        </Card.Body>
